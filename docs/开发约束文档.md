...
**用户体系约束**：
- `users` = 平台管理员（后台管理） | `application_members` = 应用成员（访问控制） | `application_users` = 业务用户（ToC用户）
- 开发时必须明确当前操作的是哪种用户类型，绝不可混淆
- 权限检查顺序：平台用户身份 → 应用访问权限 → 业务功能权限
...
## 用户体系约束
1. **严格区分三种用户类型** - 绝不可混淆用户身份和用途
   - `users` 表：平台管理员用户（登录AINO后台，管理平台和应用）
   - `application_members` 表：应用成员（控制哪些平台用户可以访问特定应用）
   - `application_users` 表：应用业务用户（ToC用户，使用应用功能的最终用户）
2. **权限层级清晰** - 三层权限控制，每层职责明确
   - 平台级权限：控制谁能登录AINO平台
   - 应用级权限：控制谁能访问特定应用
   - 业务级权限：控制应用内的业务功能权限
3. **用户身份不混淆** - 开发时必须明确当前操作的是哪种用户类型
   - 后台管理功能 → 使用 `users` 表
   - 应用访问控制 → 使用 `application_members` 表  
   - 前端业务功能 → 使用 `application_users` 表

## 多服务器部署约束
1. **生产环境数据库操作限制** - 项目需要在多台服务器中运行，其他服务器所有者不具备直接操作数据库的能力
2. **业务代码实现数据库操作** - 所有数据库操作都必须在业务代码中实现，严禁使用脚本直接操作数据库
3. **开发环境仅用于测试** - 开发服务器上的数据库仅用于测试，不代表生产环境状态
4. **表结构修改唯一入口** - 所有数据库表结构改动必须且只能在 `init-database.js` 文件中进行，严禁手动更改数据库结构
5. **部署环境隔离** - 确保代码在多服务器环境中能够正常运行，不依赖特定的数据库管理权限
