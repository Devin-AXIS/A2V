...
```
  - 后端按“键”同步接口：
    - PUT `http://localhost:3001/api/page-configs/key/{STORAGE_KEY}`
    - GET `http://localhost:3001/api/page-configs/key/{STORAGE_KEY}`
...
- **[locale]/preview 与 [locale]/preview/[id]**（预览容器）
  ...
  - 后端读取：GET `http://localhost:3001/api/preview-manifests/{id}` → `manifest`
  - 运行期处理：
...
- **[locale]/app/[appKey]**（已发布的应用运行页）
  - 后端读取：GET `http://localhost:3001/api/apps/{appKey}/manifest?state=published`
  - 路由选择：
