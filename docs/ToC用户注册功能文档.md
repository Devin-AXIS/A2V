# ToC用户注册功能文档

## 📋 功能概述

ToC用户注册功能允许前端用户通过手机号自主注册，并自动同步到后台用户列表中。支持智能合并功能，当注册用户与后台已有用户手机号相同时，会自动合并用户信息。

## 🎯 核心特性

### 1. 双重用户来源
- **后台手动添加**：管理员在后台手动创建用户
- **前端用户注册**：用户通过手机号自主注册

### 2. 智能合并机制
- **相同手机号检测**：注册时自动检查手机号是否已存在
- **自动合并**：发现相同手机号时自动合并用户信息
- **数据保留策略**：保留后台用户基础信息，添加注册用户密码和扩展信息

### 3. 用户状态管理
- **pending**：后台添加，无密码状态
- **active**：注册用户或已合并用户，有密码状态
- **inactive**：已禁用状态

## 🔧 API接口

### 用户注册接口

**接口地址**：
```
POST /api/modules/system/user/register?applicationId={applicationId}
```

**请求头**：
```
Content-Type: application/json
Authorization: Bearer {token}
```

**请求参数**：
```json
{
  "phone": "13800138000",        // 必填：手机号
  "password": "password123",     // 必填：密码（至少6位）
  "name": "张三",                // 可选：姓名
  "email": "zhangsan@example.com", // 可选：邮箱
  "gender": "男",                // 可选：性别
  "city": "北京",                // 可选：城市
  "birthday": "1990-01-01",      // 可选：生日
  "avatar": "avatar_url"         // 可选：头像URL
}
```

**响应示例**：
```json
{
  "success": true,
  "data": {
    "id": "7e87aa0e-4d7b-4bb2-b84f-54042aafe664",
    "applicationId": "355cf09a-6aea-4d0d-985f-71db6442c12c",
    "name": "张三",
    "email": "zhangsan@example.com",
    "phone": "13800138000",
    "status": "active",
    "role": "user",
    "metadata": {
      "city": "北京",
      "gender": "男",
      "source": "register",
      "password": "password123",
      "registeredAt": "2025-08-31T16:40:29.122Z"
    },
    "createdAt": "2025-08-31T16:40:29.122Z",
    "updatedAt": "2025-08-31T16:40:29.122Z"
  },
  "message": "用户注册成功"
}
```

### 用户列表查询接口

**接口地址**：
```
GET /api/modules/system/user?applicationId={applicationId}
```

**查询参数**：
- `page`: 页码（默认1）
- `limit`: 每页数量（默认20）
- `search`: 搜索关键词
- `status`: 用户状态筛选
- `role`: 用户角色筛选

**响应示例**：
```json
{
  "success": true,
  "data": {
    "users": [
      {
        "id": "7e87aa0e-4d7b-4bb2-b84f-54042aafe664",
        "name": "张三",
        "email": "zhangsan@example.com",
        "phone": "13800138000",
        "status": "active",
        "role": "user",
        "metadata": {
          "city": "北京",
          "gender": "男",
          "source": "register"
        }
      }
    ],
    "pagination": {
      "page": 1,
      "limit": 20,
      "total": 1,
      "totalPages": 1
    }
  }
}
```

## 🔄 合并逻辑详解

### 合并触发条件
当用户注册时，系统会检查是否存在相同手机号的用户：
```typescript
const existingUser = await findUserByPhone(applicationId, phone)
if (existingUser) {
  // 执行合并逻辑
  return await mergeUser(applicationId, existingUser.id, registerData)
}
```

### 合并策略
```typescript
// 合并后的用户数据结构
{
  // 保留后台用户的基础信息
  "name": "李四",                    // 后台设置
  "email": "lisi@example.com",       // 后台设置
  "phone": "13900139000",           // 合并字段
  "role": "admin",                  // 后台设置
  "department": "技术部",            // 后台设置
  "position": "工程师",              // 后台设置
  "status": "active",               // 合并后激活
  
  // 添加注册用户的扩展信息
  "metadata": {
    "password": "newpassword123",    // 注册时设置
    "gender": "女",                  // 注册时填写
    "city": "上海",                  // 注册时填写
    "source": "merged",              // 合并标识
    "mergedAt": "2025-08-31T16:40:51.321Z", // 合并时间
    "originalSource": "manual"       // 原始来源
  }
}
```

### 合并规则
1. **基础信息**：以后台用户为准（姓名、邮箱、角色、部门、职位）
2. **手机号**：使用注册用户的手机号
3. **密码**：使用注册用户的密码
4. **扩展信息**：合并注册用户的额外信息到metadata
5. **状态**：自动激活（active）
6. **来源标识**：标记为"merged"，记录合并时间

## 📊 用户数据字段说明

### 基础字段（后台可见）
- `id`: 用户ID
- `name`: 姓名
- `email`: 邮箱
- `phone`: 手机号
- `status`: 状态（active/inactive/pending）
- `role`: 角色（admin/user/guest）
- `department`: 部门
- `position`: 职位
- `tags`: 标签
- `lastLoginAt`: 最后登录时间
- `createdAt`: 创建时间
- `updatedAt`: 更新时间

### 扩展字段（存储在metadata中）
- `password`: 密码（需要加密存储）
- `gender`: 性别
- `city`: 城市
- `birthday`: 生日
- `avatar`: 头像
- `source`: 来源（register/manual/merged）
- `registeredAt`: 注册时间
- `mergedAt`: 合并时间
- `originalSource`: 原始来源

## 🛡️ 安全考虑

### 当前实现
- 密码明文存储（需要改进）
- 手机号作为唯一标识
- 自动合并机制

### 建议改进
1. **密码加密**：使用bcrypt等加密算法
2. **手机号验证**：添加短信验证码
3. **重复注册检查**：防止恶意重复注册
4. **数据验证**：加强输入数据验证

## 🧪 测试用例

### 测试场景1：新用户注册
```bash
curl -X POST "http://47.94.52.142:3001/api/modules/system/user/register?applicationId=xxx" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer test-token" \
  -d '{
    "phone": "13800138000",
    "password": "password123",
    "name": "张三",
    "gender": "男",
    "city": "北京"
  }'
```

**预期结果**：
- 创建新用户
- 状态为active
- 来源标记为register
- 出现在用户列表中

### 测试场景2：合并现有用户
```bash
# 1. 后台添加用户
curl -X POST "http://47.94.52.142:3001/api/modules/system/user?applicationId=xxx" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer test-token" \
  -d '{
    "name": "李四",
    "phone": "13900139000",
    "role": "admin",
    "department": "技术部"
  }'

# 2. 相同手机号注册
curl -X POST "http://47.94.52.142:3001/api/modules/system/user/register?applicationId=xxx" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer test-token" \
  -d '{
    "phone": "13900139000",
    "password": "newpassword123",
    "gender": "女",
    "city": "上海"
  }'
```

**预期结果**：
- 自动合并用户
- 保留后台信息（姓名、角色、部门）
- 添加注册信息（密码、性别、城市）
- 来源标记为merged
- 状态为active

## 📋 使用流程

### 前端用户注册流程
1. 用户填写注册表单（手机号、密码、其他信息）
2. 前端调用注册API
3. 后端检查手机号是否已存在
4. 如果存在：执行合并逻辑
5. 如果不存在：创建新用户
6. 返回注册结果
7. 用户自动出现在后台用户列表中

### 后台管理流程
1. 管理员查看用户列表
2. 可以看到所有用户（包括注册用户）
3. 可以编辑用户信息
4. 可以管理用户角色和权限
5. 可以查看用户来源和合并历史

## 🚀 后续优化建议

1. **密码安全**：实现密码加密存储
2. **验证码**：添加手机号验证码验证
3. **字段管理**：实现字段显示/隐藏的自定义
4. **用户登录**：实现用户登录验证功能
5. **权限控制**：完善用户权限管理
6. **数据导出**：支持用户数据导出功能
7. **批量操作**：支持批量用户管理操作

---

**重要提醒**：当前密码为明文存储，生产环境使用前请务必实现密码加密功能。
