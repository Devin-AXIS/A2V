...
export default function ClientConfigPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const u = new URL(`http://localhost:3008/${baseLang}/auth/login`)
  ...
  async function openPreview() {
    try {
      ...
      const url = `http://localhost:3008/${lang}/preview?previewId=${id}&device=${device}&appId=${params.appId}&data=${dataParam}`
      setPreviewUrl(url)
      setViewTab("preview")
  ...
  async function savePreview(bodyOverride?: any, skipMergeFromEditor?: boolean) {
    try {
      ...
      try {
        const u = new URL(previewUrl || `http://localhost:3008/${lang}/preview?previewId=${previewId}&device=${device}&appId=${params.appId}`)
        ...
        const dataParam = encodeURIComponent(JSON.stringify(draft?.dataSources || {}))
        const fallback = `http://localhost:3008/${lang}/preview?previewId=${previewId}&device=${device}&appId=${params.appId}&data=${dataParam}`
  ...
  useEffect(() => {
    const allowedChildOrigin = "http://localhost:3008"; // 调整为实际被嵌入系统的域名
  ...
  function onSwitchTab(next: "preview" | "code") {
    if (next === "preview") {
      ...
      if (previewSource === "auth-login") {
        const baseLang = (draft.app?.defaultLanguage || (lang === "zh" ? "zh" : "en")) as string
        setPreviewUrl(`http://localhost:3008/${baseLang}/auth/login`)
        setViewTab("preview")
      } else if (previewSource === "home") {
        const baseLang = (draft.app?.defaultLanguage || (lang === "zh" ? "zh" : "en")) as string
        setPreviewUrl(`http://localhost:3008/${baseLang}/home`)
        setViewTab("preview")
      } else {
        openPreview()
      }
  ...
  useEffect(() => {
    ...
    try {
      ...
      const base = String(previewUrl || `http://localhost:3008/${lang}/p/${k.replace(/^p-/, '')}`)
                              ...
                              onClick={() => {
                                ...
                                if (pageKey) {
                                  try {
                                    ...
                                    const u = new URL(`http://localhost:3008/${lang}${r}`)
                                    setPreviewUrl(u.toString())
                                  } catch {
                                    const r = item.route?.startsWith('/') ? item.route : `/${pageKey}`
                                    setPreviewUrl(`http://localhost:3008/${lang}${r}`)
                                  }
                    ...
                    <div className="flex items-center gap-2">
                      <Button variant="outline" size="sm" onClick={() => {
                        ...
                        try {
                          const baseLang = (draft.app?.defaultLanguage || "zh") as string;
                          const dataParam = encodeURIComponent(JSON.stringify(draft?.dataSources || {}))
                          const url = `http://localhost:3008/${baseLang}/preview?previewId=${previewId}&device=${device}&appId=${params.appId}&data=${dataParam}`
                          setPreviewUrl(url);
                        } catch { }
                      }}>
                                  ...
                                  setTimeout(() => {
                                    try {
                                      ...
                                      fetch('http://localhost:3007/api/page-configs', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(cfg) })
                                        ...
                                        .then(j => {
                                          const id = j && (j.id || j.data?.id)
                                          const u = new URL(`http://localhost:3008/${lang}/p/${String(pageKey).replace(/^p-/, '')}`)
                                          if (id) u.searchParams.set('cfgId', String(id))
                                          setPreviewUrl(u.toString())
                                        })
                                        .catch(() => {
