...
export const setDatas = async () => {
    ...
    Object.keys(dataMappings).forEach(async key => {
        ...
        promises.push(new Promise(async (resolve, reject) => {
            ...
            let resultData;
            if (type === "table") {
                const { data } = await axios.get(`http://47.94.52.142:3007/api/records/${dirId}?noAuth=true`);
                realData = data.data;
                resultData = [];
                ...
                const { data } = await axios.get(`http://47.94.52.142:3007/api/records/${dirId}/${recordId}?noAuth=true`)
                realData = data.data
                resultData = {};
                Object.keys(dataMapping).forEach(key => {
                    if (key.indexOf('[].') > -1) {
