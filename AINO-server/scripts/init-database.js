 ...
 * 
 * ⚠️ 多服务器部署约束：
 * - 项目需要在多台服务器中运行，其他服务器所有者不具备直接操作数据库的能力
 * - 所有数据库操作都必须在业务代码中实现，严禁使用脚本直接操作数据库
 * - 开发服务器上的数据库仅用于测试，不代表生产环境状态
 * - 所有数据库表结构改动必须且只能在此文件中进行，严禁手动更改数据库结构
 * - 确保代码在多服务器环境中能够正常运行，不依赖特定的数据库管理权限
 */
