# 🐳 AINO Docker 环境部署指南

## 📋 Docker 环境说明

当您的数据库运行在 Docker 容器中时，需要使用特殊的部署方式。本指南将帮助您在 Docker 环境中正确部署 AINO 项目。

## 🐳 Docker 数据库信息

根据您提供的信息：
- **容器名称**: `aino-postgres`
- **镜像**: `postgres:16`
- **端口映射**: `0.0.0.0:5433->5432/tcp`
- **状态**: `Up About an hour (healthy)`

## 🚀 部署步骤

### 方法一：使用修改后的部署脚本（推荐）

```bash
# 1. 克隆项目
git clone <your-repo-url>
cd AINO/AINO-server

# 2. 运行一键部署脚本（已适配Docker）
./deploy.sh
```

### 方法二：手动部署

```bash
# 1. 安装依赖
npm install

# 2. 初始化数据库（使用修改后的脚本）
./scripts/setup-database.sh

# 3. 启动服务
npm start
```

### 方法三：直接使用 Node.js 脚本

```bash
# 1. 安装依赖
npm install

# 2. 直接运行初始化脚本
node scripts/init-database.js

# 3. 启动服务
npm start
```

## 🔧 环境变量配置

确保设置正确的环境变量：

```bash
# Docker 环境配置
export DB_HOST=localhost
export DB_PORT=5433
export DB_USER=aino
export DB_PASSWORD=pass
export DB_NAME=aino
```

## 🔍 故障排除

### 1. 数据库连接失败

**检查 Docker 容器状态：**
```bash
docker ps | grep postgres
```

**检查端口映射：**
```bash
docker port aino-postgres
```

**检查容器日志：**
```bash
docker logs aino-postgres
```

### 2. 权限问题

**检查数据库用户权限：**
```bash
# 进入容器
docker exec -it aino-postgres psql -U aino -d aino

# 检查用户权限
\du
```

### 3. 网络问题

**检查网络连接：**
```bash
# 测试端口连通性
telnet localhost 5433

# 或者使用 nc
nc -zv localhost 5433
```

## 📊 Docker 环境特点

### 优势
- ✅ **隔离性**: 数据库运行在独立容器中
- ✅ **可移植性**: 易于在不同环境间迁移
- ✅ **版本控制**: 可以固定 PostgreSQL 版本
- ✅ **资源管理**: 可以限制容器资源使用

### 注意事项
- ⚠️ **端口映射**: 确保端口映射正确
- ⚠️ **数据持久化**: 确保数据卷正确挂载
- ⚠️ **网络配置**: 确保容器网络配置正确
- ⚠️ **权限设置**: 确保数据库用户权限足够

## 🛠️ Docker 命令参考

### 常用命令
```bash
# 查看运行中的容器
docker ps

# 查看所有容器
docker ps -a

# 查看容器日志
docker logs aino-postgres

# 进入容器
docker exec -it aino-postgres bash

# 重启容器
docker restart aino-postgres

# 停止容器
docker stop aino-postgres

# 启动容器
docker start aino-postgres
```

### 数据库操作
```bash
# 进入 PostgreSQL
docker exec -it aino-postgres psql -U aino -d aino

# 执行 SQL 文件
docker exec -i aino-postgres psql -U aino -d aino < init.sql

# 备份数据库
docker exec aino-postgres pg_dump -U aino aino > backup.sql

# 恢复数据库
docker exec -i aino-postgres psql -U aino -d aino < backup.sql
```

## 🎯 部署验证

部署完成后，验证以下内容：

1. **数据库连接**: 确保能够连接到 Docker 中的数据库
2. **表创建**: 确保所有表都正确创建
3. **服务启动**: 确保 AINO 服务正常启动
4. **API 测试**: 确保 API 接口正常工作

```bash
# 测试健康检查
curl http://47.94.52.142:3007/health

# 应该返回: ok
```

## 📝 常见问题

### Q: 为什么需要修改脚本？
A: 原始脚本使用 `psql` 命令，但在 Docker 环境中，PostgreSQL 客户端可能没有安装在主机上。修改后的脚本使用 Node.js 进行数据库连接，更加灵活。

### Q: 端口映射有什么要求？
A: 确保 Docker 容器的 5432 端口映射到主机的 5433 端口，这样 AINO 服务才能连接到数据库。

### Q: 如何备份 Docker 数据库？
A: 使用 `docker exec` 命令执行 `pg_dump` 进行备份，或者备份整个数据卷。

---

**🎉 现在您可以在 Docker 环境中成功部署 AINO 项目了！**
