...
/**
 * @title WorkloadToken
 * @dev 基于工作量证明的代币合约（适配Base链），提交工作量证明后按策略发放奖励
 */
contract WorkloadToken is ERC20, Ownable, ReentrancyGuard {
    struct WorkProof {
        ...
        uint256 inputSize;
        uint256 outputSize;
        uint256 executionTime; // 毫秒
        uint256 timestamp;     // 客户端生成的毫秒级时间戳
        bytes32 proofHash;
        bool verified;
    }
    ...
    function submitWorkProof(
        ...
        require(executionTime >= MIN_EXECUTION_TIME && executionTime <= MAX_EXECUTION_TIME, unicode"执行时间超出范围");
