...
# One-click deploy for workload-contracts to Base (Sepolia/Mainnet)
# Usage:
#   ./scripts/deploy-workload-base.sh                # default baseSepolia
#   ./scripts/deploy-workload-base.sh baseMainnet    # deploy to Base mainnet

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
CONTRACTS_DIR="$REPO_ROOT/BMCP/workload-contracts"
...
if command -v pnpm >/dev/null 2>&1; then
  ...
  echo "❌ 未找到 pnpm 或 npm，请先安装 Node.js/pnpm" >&2
  exit 1
fi

# compile
if [ "$PKG_MGR" = "pnpm" ]; then
  pnpm exec hardhat compile --config hardhat.config.base.js
else
...
if [ "$PKG_MGR" = "pnpm" ]; then
  pnpm exec hardhat run scripts/deploy-base.js --network "$NETWORK" --config hardhat.config.base.js
else
